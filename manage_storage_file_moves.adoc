...
=== Moving a file between two FlexGroup volume constituents in the same FlexGroup volume using the "force" option
...
# The call:
curl -X POST  "https://<mgmt-ip>/api/storage/file/moves?force=true" -H "accept: application/hal+json" -d @move_between_fg_force.txt
