...
|id
|integer
a|The identifier assigned to the vVol binding. The bind identifier is unique amongst all class `vvol` LUNs bound to the same class `protocol_endpoint` LUN.


|is_optimal
|boolean
a|Indicates if the class `procotol_endpoint` LUN and the class `vvol` LUN are on the same cluster node.
