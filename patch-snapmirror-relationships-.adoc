...
The policy cannot be changed if the policy has an "archive.after_days" value of "0" and the new policy does not have an "archive.after_days" value of "0".
...
| 6621103
| Modifying the policy is not supported because the original policy has an archive.after_days value of zero, and the new policy has a non-zero archive.after_days value.

| 13303951
| This SnapMirror policy is not supported for relationships with object store destination endpoints.
|===
