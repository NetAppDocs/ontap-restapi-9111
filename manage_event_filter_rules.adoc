...
# The call:
curl -X POST "https://<mgmt-ip>/api/support/ems/filters/test-filter/rules/1" -H "accept: application/hal+json" -H "Content-Type: application/json" -d "@test_ems_filter_rules_patch.txt"
